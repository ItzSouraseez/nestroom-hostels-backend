...
const addResidentSchema = Joi.object({
  ...
  idCardType: Joi.string().valid("Aadhaar", "PAN", "DL", "Passport", "Other").allow(null),
  ...
  checkInDate: Joi.string().isoDate().allow(null),
});
...
const attendanceConfigSchema = Joi.object({
  ...
  attendanceTime: Joi.string().pattern(/^\d{2}:\d{2}$/).required(),
  attendanceFrequency: Joi.string().valid("Daily","Weekly","Custom").default("Daily"),
  surpriseCheckEnabled: Joi.boolean().default(false),
});
